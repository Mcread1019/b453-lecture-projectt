...
func _draw():
	# Draw a larger, stretched blaster shot (visually different from billion blaster)
	var trail_length = 16.0  # Longer trail than billion blaster
	var trail_dir = Vector2(-trail_length, 0)

	# Elongated main shot (stretched ellipse effect using multiple circles)
	draw_circle(Vector2(4, 0), shot_radius * 0.8, shot_color)
	draw_circle(Vector2.ZERO, shot_radius, shot_color)
	draw_circle(Vector2(-4, 0), shot_radius * 0.8, shot_color)

	# Bright center core
	draw_circle(Vector2.ZERO, shot_radius * 0.4, shot_color.lightened(0.6))

	# Longer, thicker trail
	draw_line(trail_dir, Vector2(4, 0), shot_color.darkened(0.3), shot_radius * 2.0)
